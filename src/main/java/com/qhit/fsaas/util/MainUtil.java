...
//        System.out.println(seatsList.size());
        ...
        for (int i = 0; i < seatsList.size(); i++) {
            Seats seats = seatsList.get(i);
            if (!seats.getSColumn().equals("")) {
                seatCount[a] = seatsList.size() - 11 - a;
                seatList.add(new Seat(seats.getSid(),seats.getSColumn(), seats.getSRow(), seats.getV_window(), seats.getV_aisle(), seats.getV_gate(), seats.getV_basket(), a,seats.getTid(), seats.getFlag()));
                a++;
            }
        }*/
